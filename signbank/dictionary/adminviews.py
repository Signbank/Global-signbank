...
class VariantsListView(ListView):
    model = Gloss
    template_name = 'dictionary/admin_relations_list.html'
    ...
    def get_queryset(self):
        ...
        qs = qs.filter(annotation_idgloss__regex=r"^(.*)\-([A-Z])$")

        return qs

    def render_to_response(self, context):

        return super(VariantsListView, self).render_to_response(context)
