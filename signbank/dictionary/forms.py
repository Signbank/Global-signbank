...
class GlossSearchForm(forms.ModelForm):
    ...
    weakprop = forms.ChoiceField(label=_(u'Weak prop'),choices=NEUTRALBOOLEANCHOICES)#,widget=forms.Select(attrs=ATTRS_FOR_FORMS));
    weakdrop = forms.ChoiceField(label=_(u'Weak drop'),choices=NEUTRALBOOLEANCHOICES)#,widget=forms.Select(attrs=ATTRS_FOR_FORMS));
