...
def update_examplesentence(request, examplesentenceid):
    ...
    for dataset_language in dataset_languages:
        stc = request.POST[dataset_language.name]
        if stc != "":
            vals[dataset_language.name] = stc
...
def create_examplesentence(request, senseid):
    ...
    with atomic():
        ...
        for dataset_language in dataset_languages:
            if dataset_language.name in vals:
                ExampleSentenceTranslation.objects.create(language=dataset_language, examplesentence=examplesentence, text=vals[dataset_language.name])
...
def update_sense(request, senseid):
    ...
    for dataset_language in dataset_languages:
        ...
        if dataset_language.name not in sensetranslation_dict and dataset_language.name not in vals:
            continue
        if dataset_language.name not in sensetranslation_dict and dataset_language.name in vals:
            ...
            for tr_i, tr_v in enumerate(vals[dataset_language.name]):
                if tr_v != "":
            ...
            if dataset_language.name in sensetranslation_dict and dataset_language.name not in vals:
            ...
            elif sensetranslation_dict[dataset_language.name] != vals[dataset_language.name]:
